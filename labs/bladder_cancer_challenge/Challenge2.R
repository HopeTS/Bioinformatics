...
FDR = topTable(fit2, p.value = 0.4, number = nrow(X.train))
...
muscleInvasion = knn.cv(t(X.scale), Y.train, k = 2)
#table(true = Y.train, predicted = muscleInvasion)

#sum(muscleInvasion == Y.train)/length(Y.train)

acc = balanced.accuracy(muscleInvasion, Y.train)
acc
...
m = match(rownames(FDR), rownames(X.test))
X = X.test[m,]
X.test.scale = row.scale(X)

preds <- knn(t(X.scale), t(X.test.scale), Y.train, k = 10)

write.table(preds, row.names = FALSE)
